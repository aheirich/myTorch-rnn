...
function layer:updateOutput(input)
  ...
  -- print('(127) 1..D', D, 'Wx', Wx)
  -- print('(127) D+1..D+H H', H, 'Wh', Wh)
  
  self.output:resize(N, T, H):zero()
  local prev_h = h0
  -- print("(129) prev_h", prev_h)
  -- print("(129) input_dim", self.input_dim, "hidden_dim", self.hidden_dim)
  for t = 1, T do
    local cur_x = x[{{}, t}]
    -- print("(131) cur_x", cur_x)
    local next_h = self.output[{{}, t}]
    -- print("(132) next_h", next_h)
    -- print("(132) bias_expand", bias_expand)
    -- print("(132) Wx", Wx)
    next_h:addmm(bias_expand, cur_x, Wx)
    -- print("(133) next_h from Wx and bias", next_h)
    next_h:addmm(prev_h, Wh)
    -- print("(134) t", t, "next_h after adding prev_h Wh", next_h)
    next_h:tanh()
    -- print("(135) activation", next_h)
    prev_h = next_h
  end
