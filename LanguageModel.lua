...
function LM:sample(kwargs)
  ...
  if #start_text > 0 then
    ...
    print("(175) encoded start_text", x)
    local T0 = x:size(2)
    sampled[{{}, {1, T0}}]:copy(x)
    -- print("(177) sampled", sampled, "x", x)
    scores = self:forward(x)[{{}, {T0, T0}}]
    print("(178) scores", scores)
    first_t = T0 + 1
  else
    if verbose > 0 then
